...
use noise_functions::{Noise, Value};
...
fn value_2d_octaves_benchmark(c: &mut Criterion) {
    let mut group = c.benchmark_group("value_noise_2d_3octaves_1024x1024");
    ...
    // --- quick-noise batch ---
    {
        use quick_noise::{BatchNoise, Fbm, Grid, Value};
        let grid = Grid::<2>::new(GRID_2D, GRID_2D);
        let mut result = vec![0.0; GRID_2D_AREA];
        ...
        group.bench_function("noise-functions", |b| {
            b.iter(|| {
                for z in 0..GRID_3D {
                    for y in 0..GRID_3D {
                        for x in 0..GRID_3D {
                            result[z * GRID_3D * GRID_3D + y * GRID_3D + x] = Value
...
criterion_group!(benches, value_2d_octaves_benchmark);
