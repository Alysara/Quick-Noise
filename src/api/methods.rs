...
#[derive(Copy, Clone)]
pub struct Octave<D: NoiseDimension> {
    pub frequency: D::FVec,
    pub weight: f32,
}

impl<D: NoiseDimension> Octave<D> {
    pub fn new(frequency: D::FVec, weight: f32) -> Self {
        Self { frequency, weight }
    }

    pub fn splat(frequency: f32, weight: f32) -> Self {
        Self::new(D::FVec::splat(frequency), weight)
    }
}
