...
impl<const N: usize> SimdVariableBlendImpl for ScalarMask<N> {
    ...
    #[inline(always)]
    fn vblend_64(self, true_values: Self::VecType, false_values: Self::VecType) -> ScalarReg<N> {
        unsafe {
    ...
    #[inline(always)]
    fn vblend_32(self, true_values: Self::VecType, false_values: Self::VecType) -> ScalarReg<N> {
        unsafe {
            let mut new = ScalarReg::<N>([0; N]);
            let new_ptr = new.0.as_mut_ptr() as *mut u32;
    ...
    #[inline(always)]
    fn vblend_8(self, true_values: Self::VecType, false_values: Self::VecType) -> ScalarReg<N> {
        unsafe {
            let mut new = ScalarReg::<N>([0; N]);
            let new_ptr = new.0.as_mut_ptr();
...
impl<const N: usize> SimdSqrtImpl for ScalarReg<N> {
    ...
    fn rsqrt_f32(self) -> Self {
        Self::splat_32(1.0f32).f32_div(self.sqrt_f32())
    }
}
...
impl<const N: usize> SimdBlockShiftImpl for ScalarReg<N> {
    #[inline(always)]
    fn block_left_byte_shift<const M: i32>(self) -> Self {
    ...
    #[inline(always)]
    fn block_right_byte_shift<const M: i32>(self) -> Self {
        let mut new = Self::splat_8(0);
        for block_start in (0..N).step_by(16) {
            let block_end = block_start + 16;
