...
use quick_noise::{Fbm, GridBuilder, GridNoiseImpl, Perlin, Ridged, Value, ZeroIter};
...
fn grid_perlin_2d_benchmark(c: &mut Criterion) {
    ...
    for scale in SCALES {
        ...
        let grid = GridBuilder::<2>::new(GRID_SIZE, GRID_SIZE);
...
//
//         let grid = GridBuilder::<3>::new(GRID_SIZE, GRID_SIZE, GRID_SIZE);
...
//         b.iter(|| {
//             Batch2D::fbm::<Fbm, Perlin, 1024>(x_coords.iter_simd(), y_coords.iter_simd()).fill(&mut result);
//         });
//     });
// }

fn manual_timing_check() {
    let grid = GridBuilder::<2>::new(32, 32);
    let mut result = unsafe { SimdArray::<f32, 1024>::new_uninit() };
    let freq = 1.0 / 64.0;
